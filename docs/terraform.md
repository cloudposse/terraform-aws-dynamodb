...
| <a name="input_propagate_tags_to_replica"></a> [propagate\_tags\_to\_replica](#input\_propagate\_tags\_to\_replica) | Set to `true` to enable tags propagation on replica. Changes to tags only move in one direction: from global (source) to replica. | `bool` | `false` | no |
